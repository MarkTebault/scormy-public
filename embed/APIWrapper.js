...
let _GeneralError = "General LMS Error";
...
function doLMSInitialize()
{
   ...
   if (api == null)
   {
      const message = "Unable to locate the LMS's API Implementation.\nLMSInitialize was not successful.";
      showErrorMessage(message, _Debug);
      return "false";
   }
...
function doLMSGetValue(name)
{
   ...
   else
   {
      ...
      if (errCode != _NoError)
      {
         ...
         const message = "LMSGetValue("+name+") failed. \n"+ errDescription;
         showErrorMessage(message, _Debug);
         return "";
      }
      else
...
function doLMSCommit()
{
   ...
   if (api == null)
   {
      const message = "Unable to locate the LMS's API Implementation.\nLMSCommit was not successful.  \nPlease exit and restart course and contact you administrator about this problem.";
      showErrorMessage(message, _Debug);
      return "false";
   }
   else
...
function findAPI(win)
{
   while ((win.API == null) && (win.parent != null) && (win.parent != win))
   {
      ...
      if (findAPITries > 7) 
      {
         const message = "Error finding API -- too deeply nested.";
         showErrorMessage(message, _Debug);
         return null;
      }
...
function getAPI()
{
   ...
   if (theAPI == null)
   {
      const message = "Unable to find an API adapter"
      showErrorMessage(message, _Debug);
   }
   return theAPI
}
